...
**v0.3** — **`Xenova/bge-m3`** 임베딩(1024차, multilingual) + Hybrid 검색(**dense cosine + Qdrant BM25 sparse → RRF**) + **BGE rerank** 2단 retrieval. Qdrant payload에 `rootFolder` 포함.
...
| 경로 | 검색 방식 | 비고 |
|------|-----------|------|
| **온라인** (Obsidian / API) | Qdrant hybrid + rerank | dense(bge-m3) + BM25 sparse → RRF → rerank |
...
> vault **원본 md 전체**는 Qdrant에 없습니다. **청크 조각**(`content`, `path`, `title` …) + **dense 1024차원 벡터** + **BM25 sparse(`text`)** 만 저장됩니다.
...
flowchart TD
  ...
  H --> K[BM25 sparse<br/>qdrant/bm25 · path+title+content]
  H --> S[Qdrant dense cosine<br/>bge-m3 HNSW top-K]
  K --> F[RRF fusion · RAG_RECALL_K]
  S --> F

  F --> R[2차 Rerank · bge-reranker-v2-m3]
...
| 단계 | 역할 |
|------|------|
| **1차 Hybrid** | dense(의미) + BM25 sparse(단어) → **RRF**로 후보 **넓게** 수집 |
...
| 필드 | 설명 |
|------|------|
| **vector `dense`** | `content` 임베딩 (1024차원, cosine, HNSW) |
| **vector `text`** | `path` + `title` + `content`를 Qdrant **`qdrant/bm25`** 로 sparse 인덱싱 (IDF modifier) |
...
청킹·전처리·**벡터 스키마**(BM25 sparse 추가 등)를 바꾼 뒤에는 **`npm run index -- --full`** 로 Qdrant 컬렉션을 재생성해야 반영됩니다. (`index-manifest.json` **version** 이 바뀌면 자동 full reindex)
...
| 출력 | 내용 |
|------|------|
| **Qdrant** `company-rag` | 청크 텍스트 + **named vectors** (`dense` + sparse `text`) + payload (`path`, `title`, `content`, `startLine`, `rootFolder`) |
...
|------|---------------------|------|
| 인덱싱·dense 검색 | [Xenova/bge-m3](https://huggingface.co/Xenova/bge-m3) | `@xenova/transformers`, 1024-dim cosine, `EMBEDDING_MODEL` |
| Sparse 검색 | Qdrant `qdrant/bm25` | 서버 inference, `path+title+content` → sparse `text` |
...
| 저장 | 내용 |
|------|------|
| **Qdrant** (Docker) | 청크 + named vectors (`dense` bge-m3 + sparse BM25 `text`) |
